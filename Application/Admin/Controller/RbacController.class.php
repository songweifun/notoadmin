...
//            echo json_encode(array('result'=>$nodeList,'totalcount'=>$totalcount));
    ...
    public function manageLibrary(){
        ...
        if($action=='getLibraryList'){
            ...
            //echo $start;die;
            //$end=$page*$pagesize;
            $totalcount=count(M('library')->select());

            $result=M('library')->limit($start,$pagesize)->select();
        ...
        }elseif ($action=='addLibrary'){//增加官员


            if(M('library')->add($_POST)){
                echo json_encode(array('status'=>true,'msg'=>'添加成功'));
            }else{
                echo json_encode(array('status'=>false,'msg'=>'添加失败'));
            }
            //print_r($_POST);
            die;
        }elseif ($action=='editLibrary'){
            $id=I('get.id');
            if($id){
                if(M('library')->where(array('id'=>$id))->save($_POST)){
            ...
            die;
        }elseif ($action=='deleteLibrary'){//删除
            //echo $_GET['id'];
            $id=I('get.id');
            if(M('library')->delete($id)){
                echo json_encode(array('status'=>true,'msg'=>'删除成功'));
            }else{
                echo json_encode(array('status'=>false,'msg'=>'删除失败'));
            }
            ...
            //获得单个item信息
            $id=I('get.id');
            echo json_encode(M('library')->find($id));
            die;
        }
